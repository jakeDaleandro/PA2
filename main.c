...
void *worker_fn(void *arg)
{
    ...
    if (strcmp(op, "insert") == 0) {
        ...
        p = strtok(NULL, ",");
        if (p) strncpy(name, p, sizeof(name)-1); else name[0]='\0';
        p = strtok(NULL, ",");
        if (p) strncpy(salaryStr, p, sizeof(salaryStr)-1); else salaryStr[0]='\0';
        p = strtok(NULL, ",");
        if (p) strncpy(priorityStr, p, sizeof(priorityStr)-1); else priorityStr[0]='\0';
        ...
        if (p) strncpy(name, p, sizeof(name)-1); else name[0]='\0';
        p = strtok(NULL, ",");
        if (p) strncpy(dummy, p, sizeof(dummy)-1); else dummy[0]='\0';
        ...
        int res = deleteRecord(&table, name); // non-locking
        ...
        int res = updateSalary(&table, name, newSalary); // non-locking
        ...
        FILE *out = fopen("output.txt", "a");
        if (out) {
            if (r) fprintf(out, "Found: %u,%s,%u\n", r->hash, r->name, r->salary);
            else fprintf(out, "No Record Found: %s\n", name);
            fclose(out);
        }
    }
